...
Для таких компонент выделяется 3-4 сервера, в один пул, с идентичным окружением (ПО, переменные окружения) - например, CPP-пул с установленными студиями и .NET SDK
...
В начале это делалось в ручную, потом - автоматизировано через salt
...
## Remote-registry
TODO: Про проблему с Artifactory4 и новый форматом указания кеш-слоёв
